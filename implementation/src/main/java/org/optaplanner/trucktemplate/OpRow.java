...
public class OpRow extends AbstractPersistable implements Comparable<OpRow> {
	...
	public boolean allTypesFilled() {
		for(int i = 0; i < TruckTemplateSolution.getNumberOfTypes(); i++)
			if(sequence.getPalletCountsMax()[i] != getOpPalletsOfType(i).size()) return false;
	...
	public int countOfNeededPallets() {
		return (sequences != null) ? sequence.getCountPerType().values().stream().collect(Collectors.summingInt(Integer::intValue)) : 0;
	}
	
	public int getNeedOfType(int type) {
		return sequence.getCountPerType().get(type);
	}
	
	public Set<Integer> getNeededTypes() {
		return (sequences != null) ? sequence.getCountPerType().keySet() : Collections.emptySet();
	}

	public List<OpSequence> getListAfterCurrentSequence() {
		return sequences.stream().filter(s -> s.compareTo(sequence) == 1).toList();
	}
	...
	public String toString() {
		return "Row: " + getId() + " " + isValid() + " " + getCurrentWeight();
	}
